...
/**
 * @brief Computes the precision-recall for multi-label classification task.
 */
template <typename Dtype>
class PrecisionRecallLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  int top_k_; // select labels with top k probabilities as predictions
  int test_interval_;
  int train_iter_;
  string reps_folder_;
};
...
template <typename Dtype>
class DotLossLayer : public LossLayer<Dtype> {
 public:
  explicit DotLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), nrma_(), nrmb_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
...
template <typename Dtype>
class RankHingeLossLayer : public LossLayer<Dtype> {
 public:
  explicit RankHingeLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_RANK_HINGE_LOSS;
  }
...
template <typename Dtype>
class SoftmaxMultiLabelLossLayer : public LossLayer<Dtype> {
 public:
